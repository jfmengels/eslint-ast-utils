...
[espree, babel].forEach(({name, utils}) => {
	...
	test(`(${name}) should return false if node is not a CallExpression`, t => {
	...
	test(`(${name}) should return false if node is a CallExpression without an Identifier as the callee`, t => {
	...
	test(`(${name}) should return false if node is a CallExpression with an Identifier not named \`require\``, t => {
		t.false(lib.isStaticRequire(utils.expression(`foo('lodash')`)));
		t.false(lib.isStaticRequire(utils.expression(`bar('lodash')`)));
	});

	test(`(${name}) should return false if node is a CallExpression with more or less than 1 argument`, t => {
	...
	test(`(${name}) should return false if node is a CallExpression with an argument that is not a Literal string`, t => {
		t.false(lib.isStaticRequire(utils.expression(`require(['lodash'])`)));
	...
	test(`(${name}) should return true if node is a CallExpression on \`require\` with one string Literal argument`, t => {
